...
USER_AGENT = "fusion-mcp-client/1.0"
...
def format_item_summary(item: dict, suppliers: list = None) -> str:
    """Format a single item into a clean summary with supplier information."""
    
    fields = {
        "Item ID": item.get('ItemId', 'N/A'),
        "Item Number": item.get('ItemNumber', 'N/A'),
        "Description": item.get('ItemDescription', 'N/A'),
        "Item Class": item.get('ItemClass', 'N/A'),
        "Status": item.get('ItemStatusValue', 'N/A'),
        "Primary UOM": item.get('PrimaryUOMValue', 'N/A'),
        "List Price": f"${item.get('ListPrice')}" if item.get('ListPrice') else "N/A",
        "Item Type": item.get('UserItemTypeValue', 'N/A'),
        "Purchasable": "Yes" if item.get('PurchasableFlag') else "No",
        "Accepts Orders": "Yes" if item.get('CustomerOrderFlag') else "No",
        "Shippable": "Yes" if item.get('ShippableFlag') else "No",
        "Stock Managed": "Yes" if item.get('StockEnabledFlag') else "No",
        "Assemble to Order": "Yes" if item.get('AssembleToOrderFlag') else "No",
        "Web Status": item.get('WebStatusValue', 'N/A'),
        "Min Order Qty": item.get('MinimumOrderQuantity', 'N/A')
    }
    ...
    if suppliers:
        ...
        for supplier in suppliers:
            supplier_id = supplier.get('SupplierId', 'N/A')
            supplier_name = supplier.get('SupplierName', 'N/A')
            supplier_number = supplier.get('AlternateSupplierNumber', 'N/A') 
            status = supplier.get('SupplierSiteAssociationStatusValue', 'N/A')
            address = supplier.get('AddressName', 'N/A')
            
            supplier_info = f"  • Name: {supplier_name}\n    ID: {supplier_id}\n    Number: {supplier_number}\n    Status: {status}\n    Address: {address}"
            formatted_lines.append(supplier_info)
            
            
            sites = supplier.get('sites', [])
            if sites:
                formatted_lines.append("    Available Procurement BUs:")
                ...
                for site in sites:
                    ...
                    site_groups[site_name].append(site)
                
                for site_name, site_list in list(site_groups.items())[:3]:
                    if len(site_list) == 1:
                        site = site_list[0]
                        bu_name = site.get('ProcurementBU', 'N/A')
                        bu_id = site.get('ProcurementBUId', 'N/A')
                        ...
                        formatted_lines.append(site_info)
                        
                        # Add inventory organizations if available - only show the item's specific organization
                        inventory_orgs = site.get('inventory_organizations', [])
                        inventory_locations = site.get('inventory_locations', {})
                        
                        if inventory_orgs:
                            # Filter to show only the organization that matches the item's OrganizationId
                            item_org_id = item.get('OrganizationId')
                            matching_orgs = [org for org in inventory_orgs if str(org.get('OrganizationId')) == str(item_org_id)]
                            
                            if matching_orgs:
                                formatted_lines.append("        Destination Organizations:")
                        ...
                        for site in site_list[:3]:
                            ...
                            formatted_lines.append(f"        • Business Unit: {bu_name}\n          BU ID: {bu_id}\n          Purpose: {purpose_text}\n          Status: {site_status}")
            else:
                formatted_lines.append("    Sites: No sites found")
    else:
        formatted_lines.append("Suppliers: No suppliers found")
